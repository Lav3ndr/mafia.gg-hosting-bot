    ...
    /**public void setSetupOld(String setupCode) {
        try {
            try {
                ...
                session.findElement(By.xpath("//input[@placeholder='Paste setup codeâ€¦']"))
                        .sendKeys(Keys.chord(Keys.CONTROL, "a", Keys.DELETE));
                session.findElement(By.xpath("//input[@placeholder='Paste setup codeâ€¦']")).sendKeys(setupCode);
            }
            try {
                save();
            } catch (NoSuchElementException e) {
...
//          this.changeRoomName("8 - Man Consigliere");
    ...
    public void setDeckOld (String name) {// not working yet
        ...
        if (name.equalsIgnoreCase("no deck")) {
            ...
            session.findElement(By.xpath("//input[@placeholder='Filter decks by nameï¿½']")).sendKeys(name);
            try {
                Thread.sleep(1000);
            } catch (InterruptedException e) {
                e.printStackTrace();
